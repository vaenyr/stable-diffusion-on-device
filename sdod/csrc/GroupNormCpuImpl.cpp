...
Qnn_ErrorHandle_t execute(CustomOp* operation) {
  ...
  for (uint32_t i=0; i<input->rank; ++i) {
    size *= (size_t)input->currentDimensions[i];
  }
  ...
  // uint32_t spatial = (uint32_t)(input->currentDimensions[1] * input->currentDimensions[2]);
  // uint32_t input_channels = (uint32_t)(input->currentDimensions[3]);
